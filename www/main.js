...
function generateSysId() {
  return crypto.subtle
    .digest("MD5", new TextEncoder().encode(crypto.randomUUID()))
    .then((buffer) => {
...
function getBaseUpdateSetName(root, baseUpdateSetId) {
  for (const updateSet of root.querySelectorAll("sys_remote_update_set")) {
    if (
      updateSet.querySelector("sys_id").textContent ===
      baseUpdateSetId.textContent
    ) {
...
async function createOrRetrieveParentUpdateSet(
  baseUpdateSet,
  root,
  newBaseSetNameParent,
  createdByEmail
) {
  if (!baseUpdateSet) {
    ...
    const parentRemoteBaseUpdateSet = document.createElement(
...
function updateSysRemoteUpdateSetElements(
  root,
  matchedApplications,
  parentSysIdString
) {
  for (const updateSet of root.querySelectorAll("sys_remote_update_set")) {
    ...
    if (application && matchedApplications.has(application.textContent)) {
      ...
      const remoteBaseUpdateSet = updateSet.querySelector(
        "remote_base_update_set"
      );
...
async function generateChildUpdateSets(
...
function updateRemoteUpdateSetElements(root, applicationIdMap) {
  for (const updateXml of root.querySelectorAll("sys_update_xml")) {
    ...
    if (applicationElement && applicationElement.textContent) {
      ...
      if (updateSetId) {
        const remoteUpdateSetElement =
          updateXml.querySelector("remote_update_set");
...
function generateUpdateSetNames(updateSetName) {
  ...
  const newBaseSetNameParent =
    updateSetName.slice(0, baseNameLength) + suffixParent;
  const newBaseSetNameChild =
    updateSetName.slice(0, baseNameLength) + suffixChild;
...
function processFile() {
  ...
  reader.onload = async function (event) {
    try {
      ...
      const createdByEmail = prompt(
        "Enter the email address to set the 'created_by' field on any generated update sets:"
      );
      const applicationNames = getApplicationsReferencedInUpdates(xmlDoc);
      logToPre(
        "The following applications were found in the updates contained within this XML: " +
          Array.from(applicationNames)
      );

      const applicationsFromUpdateSets = new Set(
        Array.from(
          xmlDoc.querySelectorAll("sys_remote_update_set application")
        ).map((elem) => elem.textContent)
      );
      const unmatchedApplications = new Set(
        [...applicationNames].filter(
          (app) => !applicationsFromUpdateSets.has(app)
        )
      );
      ...
      const [baseUpdateSetId, updateSetCount] =
        getBaseUpdateSetIdFromUpdateSets(xmlDoc);
      const baseUpdateSetName = baseUpdateSetId
        ? getBaseUpdateSetName(xmlDoc, baseUpdateSetId)
        : inferBaseUpdateSetNameFromSingleUpdateSet(xmlDoc);
      logToPre(updateSetCount + " update set(s) found in the XML.");
      logToPre("Original Base Update Set Name: " + baseUpdateSetName);

      const [newParentName, newChildName] =
        generateUpdateSetNames(baseUpdateSetName);
      ...
      const matchedApplications = new Set(
        [...applicationNames].filter((app) =>
          applicationsFromUpdateSets.has(app)
        )
      );
    ...
    //  logToPre((new XMLSerializer).serializeToString(xmlDoc))
      ...
      logToPre("Update Remote Update Set Elements: " + parentSysIdString);
      updateSysRemoteUpdateSetElements(
        xmlDoc,
        matchedApplications,
        parentSysIdString
      );

      const applicationIdMap = await generateChildUpdateSets(
        unmatchedApplications,
        xmlDoc,
        newChildName,
        parentSysIdString,
